              ...
              children: [
                ...
                Expanded(
                  child: StreamBuilder<QuerySnapshot<UserModel>>(
                    ...
                    stream: MyDataBase.getUserRealTimeUpdate(userId),
                  ),
                ),
              ],
            ),
            ...
            children: [
              StreamBuilder<QuerySnapshot<UserModel>>(
                builder: (context, snapshot) {
                  ...
                  final illnessNum = userList![0].illnessNum;

                  return StreamBuilder<QuerySnapshot<SectionsModel>>(
                    builder: (context, snapshot) {
                      if (snapshot.hasError) {
                        return Text(snapshot.error.toString());
                      }
                      ...
                      var sectionList = snapshot.data?.docs.map((doc) => doc.data()).toList();
                      ...
                      return Column(
                        children: sectionList!.map((section) {
                          ...
                          return Column(
                            children: [
                              InkWell(
                                child: SectionReview(sectionsName: "مقترحات"),
                                onTap: () {
                                  Navigator.push(
                                    context,
                                    MaterialPageRoute(
                    ...
                    stream: MyDataBase.getSectionsRealTimeUpdateForRecommend(illnessNum),
                  );
                },
                stream: MyDataBase.getUserRealTimeUpdate(userId ?? ""),
              ),
              ...
              StreamBuilder<QuerySnapshot<SectionsModel>>(
                builder: (context, snapshot) {
                  if (snapshot.hasError) {
                    return Text(snapshot.error.toString());
                  }
                    ...
                    children: sectionList!.map((section) {
                      ...
                      return Column(
                        children: [
                          InkWell(
                            child:
                                SectionReview(sectionsName: section.name ?? ""),
                            onTap: () {
                              print(section.name);
                              Navigator.push(
                                context,
                                MaterialPageRoute(
                                  builder: (context) =>
                                      SectionAllProduct(sectionsModel: section),
                                ),
                              );
                            },
                          ),
